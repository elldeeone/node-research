...
- helper txgen runs as `nbs-helper-txgen.service`
...
```bash
TX_GEN_PRIVATE_KEY=...
TX_GEN_SOURCE_ADDRESS=kaspadev:...
```
...
sudo systemctl enable nbs-helper-txgen
```

The helper txgen unit is typically enabled but left stopped until the calibration or baseline load window begins.
